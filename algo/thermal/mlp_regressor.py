...
class MLPRegressionParams(BaseAlgoParams):
    ...
    def serialize(self):
        ...
        dict.__init__(self, MLPRegressionParams = {"train_csv_path":self.train_csv_path, "test_csv_path" :self.test_csv_path, "hidden_layer_sizes":self.hidden_layer_sizes,"activation":self.activation, "solver":self.solver, "learning_rate" :self.learning_rate, "learning_rate_init":self.learning_rate_init,"power_t":self.power_t, "alpha":self.alpha, "max_iter" :self.max_iter, "early_stopping":self.early_stopping,"warm_start":self.warm_start, "cv":self.cv})
    ...
    def deserialize(self, data):
        ...
        dict.__init__(self, train_csv_path = self.train_csv_path, test_csv_path = self.test_csv_path, hidden_layer_sizes = self.hidden_layer_sizes,activation = self.activation, solver = self.solver, learning_rate = self.learning_rate, learning_rate_init = self.learning_rate_init, power_t = self.power_t, alpha = self.alpha, max_iter = self.max_iter, early_stopping=self.early_stopping, warm_start = self.warm_start, cv = self.cv)
...
class MLPRegression(BaseAlgo):
    ...
    def run(self, input = None):
        ...
        if loaded is None:
            ...
            [r2_score, cv_results, best_params] = MLP_Regressor(param_grid, self.AlgoParams.cv, x_trainscaled, y_train, x_testscaled, y_test)
