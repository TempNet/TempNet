...
class MLPClassificationParams(BaseAlgoParams):
    ...
    def serialize(self):
        ...
        dict.__init__(self, MLPClassificationParams={"train_csv_path":self.train_csv_path, "test_csv_path" :self.test_csv_path, "hidden_layer_sizes":self.hidden_layer_sizes,"activation":self.activation,"max_iter":self.max_iter})

    def deserialize(self, data):
        super(MLPClassificationParams, self).deserialize(data)
        self.train_csv_path = data["AlgoParams"]["MLPClassificationnParams"]["train_csv_path"]
        ...
        self.max_iter = data["AlgoParams"]["MLPClassificationParams"]["max_iter"]


        dict.__init__(self, train_csv_path=self.train_csv_path, test_csv_path= self.test_csv_path, hidden_layer_sizes=self.hidden_layer_sizes,activation=self.activation, max_iter=self.max_iter)
...
class MLPClassification(BaseAlgo):
    ...
    def run(self, input = None):
        ...
        if loaded is None:
            ...
            hidden_layer_sizes = tuple(int(num) for num in self.AlgoParams.hidden_layer_sizes.replace('(', '').replace(')', '').replace('...', '').split(','))
            #Load data:
            (x_train, y_train, x_test, y_test) = load_for_mlp(self.AlgoParams.train_csv_path, self.AlgoParams.test_csv_path)
            (x_trainscaled, x_testscaled) = scaling(x_train, x_test)
      
            clf=MLPClassifier(hidden_layer_sizes=hidden_layer_sizes,activation=self.AlgoParams.activation,max_iter=self.AlgoParams.max_iter)
        ...
        # algo output dict
        mlp_classifier_output_dict = {'score': score,'classification_report':classification_report,'Confusion_Matrix':Confusion_Matrix}
